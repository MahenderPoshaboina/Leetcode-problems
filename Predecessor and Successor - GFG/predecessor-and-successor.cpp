...
public:
    void findPreSuc(Node *root, Node *&pre, Node *&suc, int key)
    {
        ...
        if (root->key == key)
        {
            // If the current node is the key node
            // Predecessor will be the rightmost node in the left subtree (if exists)
            if (root->left != NULL)
            {
                Node *temp = root->left;
            ...
            // Successor will be the leftmost node in the right subtree (if exists)
            if (root->right != NULL)
            {
        ...
        else
        {
            ...
            // Update the predecessor to the current node and move to the right subtree
            pre = root;
